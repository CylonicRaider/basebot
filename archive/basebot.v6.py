    ...
    def connect(self, _n=6, _exc=None):
        ...
        try:
            room_format = os.environ.get('BASEBOT_ROOM_FORMAT',
                'wss://euphoria.io/room/%s/ws')
            self.conn = websocket.create_connection(room_format %
                                                    self.roomname)
        except WSException:
...
class MiniBot(ThreadedBot):
    ...
    def handle_chat(self, info, message):
        ...
        if hasattr(self.regexes, 'keys'):
            for k in self.regexes:
                self._process_pair(k, self.regexes[k],
                                   info, message, True)
        else:
            for el in self.regexes:
                self._process_pair(el[0], el[1:], info, message,
                                   False)
    def _process_pair(self, key, value, info, message, is_dict):
        ...
        info['self'] = self
        if is_dict and not isinstance(value, tuple):
            fargs = (value,)
        else:
            fargs = value
        if callable(key):
...
def run_minibot(args, **config):
    ...
    regexes   : A list/mapping of regular expressions with replacements
                ...
                - A list of tuples, discriminated by the first element:
                  ...
                  - Functions are called with the info dictionary of the
                    message (with the MiniBot instance stored under key
                    "self") as the first argument, and all remaining elements
                    of the tuple as subsequent arguments. If the function
                    returns None, the next element is checked, otherwise, the
                    return value is sent as a reply.
                - A mapping:
                  - If the key is a string or a regular expression object,
                    the corresponsing value is used as a reply (similarly to
                    above).
    ...
    long_help : Long help; displayed as a response to a !help @nickname"
                command; if this is not defined, but help_sh is, the latter
                is used for both comamnds.
    """
